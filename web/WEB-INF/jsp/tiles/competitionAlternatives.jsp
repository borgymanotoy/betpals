...
<script type="text/javascript">
   function saveAndExit() {
		var alternative_count = getAlternativeListItemsCount();
		if(alternative_count > 1){
			jQuery('#manageCompetitionForm').submit();
		}else{
			$("#p_error").hide();
			$("#alternativeNameDuplicateMsg").hide();
			$("#alternativeErrorMsg").show();
			$("#name").focus();
		}
   } 
   ...
   function moveAlternative(alternativeId, direction) {
	...
	var getAlternativeListItemsCount = function(){
		var alternative_count = $("#alternativeUList li").size();
		return alternative_count;
	};

   jQuery(document).ready(function() {
		if($("#alternativeErrorMsg").is(":visible")){
			$("#alternativeErrorMsg").hide();
		}
	   var duplicate = '${alreadyNameDuplicate}';
	   if(duplicate=='true'){
		   $("#alternativeNameDuplicateMsg").show();
	   }else{
		   $("#alternativeNameDuplicateMsg").hide();
	   }
       var thumb = $('img#thumb'); 
...
<form:form commandName="alternative" action="${actionURL}" method="post" enctype="multipart/form-data" id="alternativeForm">
    <div>
		<span id="alternativeErrorMsg" class="error"><spring:message code="error.multiple.alternatives"/></span>
        <div class="span-12">
        ...
        <div class="span-12">
            <div class="span-2 labelDiv">&nbsp;</div>
            <div class="span-10 last">
				<span id="alternativeNameDuplicateMsg" class="error hide"><spring:message code="error.duplicate.alternative.name"/></span>
                <p id="p_error" class="error"><form:errors path="*"/></p>
	        ...
	        <div class="span-12">
		        ...
		        <c:when test="${status.last}">
                    <div class="span-10 last altListDetails noborder">
                        <div class="span-4 altListName">${altFromList.name}</div>
